...
use tenders::tender_mts::TenderMts;
...
impl<'a> ParserMts<'a> {
    ...
    fn parser_tender(&mut self, tender: Node) {
        ...
        let datepb_t = match regextools::RegexTools::get_one_group(&date_pb.text().trim(), r"(\d{2}\.\d{2}\.\d{4})") {
            Some(pn) => pn,
            None => {
        ...
        let tn: TenderMts = TenderMts { type_fz: 131, etp_name: "ПАО «Мобильные ТелеСистемы»".to_string(), etp_url: "https://tenders.mts.ru/".to_string(), href, pur_num, pur_name: pur_name.to_string(), date_pub, date_end, connect_string: &self.connect_string };
