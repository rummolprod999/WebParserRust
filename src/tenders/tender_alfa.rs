...
impl<'a> WebTender for TenderAlfa<'a> {
...
impl<'a> TenderAlfa<'a> {
    fn get_org_id(&self, pool: &my::Pool) -> Result<u64, Box<dyn error::Error>> {
        ...
        if let Some(org_row_t) = res.next() {
            ...
            let phone = "+7 495 755-58-58";
            let email = "mail@alfabank.ru";
            let inn = "";
            let post_address = "107078, Москва, ул. Каланчевская, 27";
            let cont_person = "";
