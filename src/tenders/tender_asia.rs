...
impl<'a> TenderAsia<'a> {
    fn get_org_id(&self, pool: &my::Pool) -> Result<u64, Box<dyn error::Error>> {
        ...
        if let Some(org_row_t) = res.next() {
            ...
            let phone = "8 (800) 550-55-66";
            let email = "info@asiacement.ru";
            let inn = "";
            let post_address = "440000, г. Пенза, ул. Бакунина, 20Б";
            let cont_person = "";
