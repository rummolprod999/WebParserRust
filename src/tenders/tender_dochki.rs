...
impl<'a> WebTender for TenderDochki<'a> {
...
impl<'a> TenderDochki<'a> {
    fn get_org_id(&self, pool: &my::Pool) -> Result<u64, Box<dyn error::Error>> {
        ...
        if let Some(org_row_t) = res.next() {
            ...
            let phone = "+7 (495) 660-51-66";
            let email = "tender@detoc.ru";
            let inn = "";
            let post_address =
                "г. Москва,проспект Севастопольский, д. 56/40, строение 1, помещение I";
            let cont_person = "";
