...
impl<'a> WebTender for TenderMegafon<'a> {
...
impl<'a> TenderMegafon<'a> {
    fn get_org_id(&self, pool: &my::Pool) -> Result<u64, Box<dyn error::Error>> {
        ...
        if let Some(org_row_t) = res.next() {
            ...
            let phone = "";
            let email = "tender@megafon-retail.ru";
            let inn = "";
            let post_address = "Москва, ул. Оружейный переулок, д. 41";
            let cont_person = "";
    ...
    fn insert_attachment(
        ...
        let attachments = doc.find(Name("a").and(Class("b-infoblock__inner")).and(|x: &Node| {
            if x.text().contains("Скачать документацию") {
                true
            } else {
                false
            }
        ...
        for att in attachments {
            ...
            if let Some(url_att) = url_att_t {
                let url_at = format!("http://corp.megafon.ru{}", url_att);
