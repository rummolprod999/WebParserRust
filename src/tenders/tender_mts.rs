...
use std::error;
use chrono::prelude::*;
use toolslib::datetimetools::DateTimeTools;
use chrono::{DateTime};
...
impl<'a> WebTender for TenderMts<'a> {
    ...
    fn parser_unwrap(&self) -> Result<(i32, i32), Box<error::Error>> {
        ...
        let id_etp = (self.get_etp_id(&pool, "ПАО «Мобильные ТелеСистемы»", "https://tenders.mts.ru/"))?;
        let id_tender = (self.get_tender_id(&pool, &id_organizer, &id_placing_way, &id_etp, &date_upd, &cancel_status))?;
        if update {
            upd_t = 1;
        } else {
            add_t = 1;
...
impl<'a> TenderMts<'a> {
    fn get_org_id(&self, pool: &my::Pool) -> Result<u64, Box<error::Error>> {
        let org_name = "ПАО «Мобильные ТелеСистемы»";
        let mut res = (pool.prep_exec("SELECT id_organizer FROM organizer WHERE full_name = :full_name", params! {"full_name" => org_name}))?;
    ...
    fn get_cus_id(&self, pool: &my::Pool) -> Result<u64, Box<error::Error>> {
        let cus_name = "ПАО «Мобильные ТелеСистемы»";
        ...
        if let Some(id_etp_row) = res.next() {
            let id_etp = (id_etp_row)?.get_opt::<Value, usize>(0).ok_or("None id_customer")?.and_then(|x| my::from_value_opt::<u64>(x))?;
            return Ok(id_etp);
        } else {
            let inn = "7740000076";
