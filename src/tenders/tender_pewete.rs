...
impl<'a> TenderPewete<'a> {
    fn get_org_id(&self, pool: &my::Pool) -> Result<u64, Box<error::Error>> {
        ...
        if let Some(org_row_t) = res.next() {
            ...
            let phone = "8(495)780-73-27";
            let email = "PWT@pewete.com";
            let inn = "7706425914";
            let post_address = "119180, г. Москва, 2-ой Казачий пер., д.4";
            let cont_person = "";
