...
impl<'a> WebTender for TenderQuadra<'a> {
...
impl<'a> TenderQuadra<'a> {
    fn get_org_id(&self, pool: &my::Pool) -> Result<u64, Box<dyn error::Error>> {
        ...
        if let Some(org_row_t) = res.next() {
            ...
            let phone = "+7 (4872) 25-43-59";
            let email = "tula@quadra.ru";
            let inn = "6829012680";
            let post_address =
                "Российская Федерация, 300012, Тульская обл., г. Тула, ул. Тимирязева, д. 99в";
            let cont_person = "";
