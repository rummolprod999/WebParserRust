...
impl<'a> WebTender for TenderRuscoal<'a> {
...
impl<'a> TenderRuscoal<'a> {
    fn get_org_id(&self, pool: &my::Pool) -> Result<u64, Box<dyn error::Error>> {
        ...
        if let Some(org_row_t) = res.next() {
            ...
            let phone = "+7 (495) 225-25-05";
            let email = "reception@ruscoal.ru";
            let inn = "";
            let post_address = "107031, г.Москва, ул. Петровка, д.10";
            let cont_person = "";
