...
pub trait WebTender {
    ...
    fn ret_cancel_status(&self, pool: &my::Pool, type_fz: i32, pur_num: &String, date_upd: &DateTime<FixedOffset>) -> Result<(i32, bool), Box<error::Error>> {
                ...
                Ok(res) => {
                    ...
                    match d_upd.cmp(&date_version) {
                        Ordering::Greater | Ordering::Equal => {
                            let _res_upd = (pool.prep_exec("UPDATE tender SET cancel=1 WHERE id_tender = :id_tender", params! {"id_tender" => &id_tender}))?;
                        }
                        Ordering::Less => {
                            cancel = 1;
                        }
    ...
    fn get_etp_id(&self, pool: &my::Pool, etp_name: &str, etp_url: &str) -> Result<u64, Box<error::Error>> {
        let mut res = (pool.prep_exec("SELECT id_etp FROM etp WHERE name = :name AND url = :url LIMIT 1", params! {"name" => etp_name, "url" => etp_url}))?;
        if let Some(id_etp_row) = res.next() {
            let id_etp = (id_etp_row)?.get_opt::<Value, usize>(0).ok_or("None id_etp")?.and_then(|x| my::from_value_opt::<u64>(x))?;
            return Ok(id_etp);
        } else {
            let res_insert = (pool.prep_exec("INSERT INTO etp SET name = :name, url = :url, conf=0", params! {"name" => etp_name, "url" => etp_url}))?;
            return Ok(res_insert.last_insert_id());
        }
    }

    fn add_version_num(&self, pool: &my::Pool, type_fz: i32, pur_num: &String) -> Result<(), Box<error::Error>> {
        let mut ver_num = 1;
        ...
        for r in res.into_iter() {
            let id_tender = (r)?.get_opt::<Value, usize>(0).ok_or("None id_tender in add_ver_number")?.and_then(|x| my::from_value_opt::<u64>(x))?;
    ...
    fn add_tender_keywords(&self, pool: &my::Pool, id_tender: &u64) -> Result<(), Box<error::Error>> {
        let mut s = "".to_string();
        ...
        let res_att = (pool.prep_exec("SELECT DISTINCT file_name FROM attachment WHERE id_tender = :id_tender", params! {"id_tender" => id_tender}))?;
        for r_att in res_att.into_iter() {
            let name_att = (r_att)?.get_opt::<Value, usize>(0).ok_or("None name in attachment")?.and_then(|x| my::from_value_opt::<String>(x)).unwrap_or_default();
        ...
        for r_ten in res_ten.into_iter() {
            ...
            let id_org = r_p.get_opt::<Value, usize>(1).ok_or("None id_org in tender")?.and_then(|x| my::from_value_opt::<u64>(x)).unwrap_or_default();
            if id_org != 0 {
        ...
        (pool.prep_exec("UPDATE tender SET tender_kwords = :tender_kwords WHERE id_tender = :id_tender", params! {"tender_kwords" => &res_string.trim(), "id_tender" => &id_tender}))?;
        Ok(())
    }
}